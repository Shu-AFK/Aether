...
void *paging_align_address(void *ptr) {
    if((uint32_t) ptr % PAGING_PAGE_SIZE != 0) {
...
int paging_map(uint32_t *directory, void *virt, void *phys, int flags) {
    if(((unsigned int) virt % PAGING_PAGE_SIZE != 0) || ((unsigned int) phys % PAGING_PAGE_SIZE != 0)) {
        return -EINVARG;
    }
...
int paging_set(uint32_t *directory, void *virtual_addr, uint32_t val) {
    ...
    uint32_t *table = (uint32_t *) (entry & 0xfffff000); // Extracting the page table address
